...
- `components/ContinueToBookmark.tsx` — home-page bookmark card (see `docs/features/bookmarking.md` for the bookmark mechanism itself).
...
- Connected from: `ProgressTracker` is the only writer of reading-progress state; it's mounted once per reader client component (`ReaderClient`, `ParallelReaderClient`, `GradedReaderClient` — not `PdfReaderClient`, which tracks its own page/scroll position instead, see `docs/features/pdf-technical-reader.md`).
